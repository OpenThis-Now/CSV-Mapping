...
def get_project_stats(project_id: int, session: Session = Depends(get_session)) -> dict:
    ...
    not_approved_count = status_counts.get("not_approved", 0) + status_counts.get("auto_not_approved", 0) + status_counts.get("rejected", 0) + status_counts.get("ai_auto_rejected", 0)
